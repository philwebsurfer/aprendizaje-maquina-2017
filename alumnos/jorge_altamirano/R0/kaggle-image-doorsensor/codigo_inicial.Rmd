...
# date()
  ...
  # df_train2$estado <- as.factor(ifelse(df_train2$estado == "abierta", 1, 0))
  # date()
...
mod_ridge_cv <- cv.glmnet(x = df_train2[,c(1:855,857)] %>% data.matrix,
  y = df_train2$estado,
  alpha = 0, #ridge
  nfolds = 50, lambda = exp(seq(-5,2,1)),
  family='binomial', intercept = T, nlambda=50) #normalmente ponemos intercept = T
mod_ridge_cv$lambda.1se
...
preds_ridge <- predict(mod_ridge_cv, lambda=mod_ridge_cv$lambda.min, 
         newx = df_train2[,c(1:855,857)] %>% data.matrix,
        type = 'response')
...
```{r ROCR}
pred <- prediction(preds_ridge, df_train$estado)
perf <- performance(pred, measure='sens', x.measure ='fpr')
table(preds_ridge_se > 0.5, df_train$estado)
prop.table(table(preds_ridge_se > 0.45, df_train$estado), 2)
plot(perf, col='green')
```
